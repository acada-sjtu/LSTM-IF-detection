...
class Supervisor:
	...
	def launch(self, benchmark):
		self.rGenerator.reset()
		randomnum = 1 # input("\nPlease input number of combinations.\n")
		length = input("\nPlease input the length of the time series you want to run? [50 is recommended]\n")
		# length = 50
		self.randomnum = int(randomnum)
		self.length = int(length)
		self.launchNumberedErrors(benchmark)
	...
	def launchNumberedErrors(self, benchmark):
		...
		errorCount = input("\nPlease input the number of errorVectors you want to simulate\n>")
		self.errorCount = int(errorCount)
		# singleError = raw_input("\nSingle fault or not? [y/n]")
		singleError = 'y'
		if singleError == "y":
			self.launchNumberedErrorsCombinations(1, benchmark)
		else:
			self.launchNumberedErrorsCombinations(self.limits[1] - 1, benchmark)

	def launchNumberedErrorsCombinations(self, k, benchmark):
		recur = 0
		insertPos = input("\nPlease input the insert position. [f/m/e]\n")
		error_ratio = 0.3
		fin = open('inputdata_%s_%s_%.1f'%(benchmark, insertPos,error_ratio), 'a+')
		...
		while self.valid_sample < self.errorCount:
			...
			self.launchEntriesCombinations(k, benchmark, insertPos, error_ratio)
			print "recur:", recur, "# of valid sample:", self.valid_sample
 
	def launchEntriesCombinations(self, k, benchmark, insertPos='f',error_ratio=0.3):
		begin_cycle = self.length * self.insertion_map[insertPos]
		...
		for self.entryCount in range(self.randomnum):
			...
			self.error_count = 0
			fin = open('inputdata_%s_%s_%.1f'%(benchmark, insertPos, error_ratio), 'a+')
			ferror = open('inputerror_%s_%s_%.1f'%(benchmark, insertPos, error_ratio), 'a+')
			# flipped = open('flipped_%s_%s'%(benchmark, insertPos), 'a+')
			affected = open('affected_%s_%s'%(benchmark, insertPos), 'a+')
